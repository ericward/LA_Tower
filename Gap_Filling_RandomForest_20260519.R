...
##### CH4 Gap Filling #####
colnames(la2_hydro_climate)
str(la2_hydro_climate)
la2_reduced<-la2_hydro_climate %>% filter (DateTime > "2021-01-01*") %>% select(DateTime,`Adjusted Water Level`,FC,FCH4,RH, TA,
...
ggplot(la2_reduced, aes(x = DateTime, y = FCH4)) + geom_point()
...
na_gaps <- la2_reduced %>% filter(DateTime > "2021-01-01") %>%
  ...
  filter(!is.na(gap_start) & gap_length != 0)  # keep only NA groups

short_gaps<- na_gaps %>% filter(gap_length < 24)

#------------------------------------------------------------------------------
# 1. GAP-FILL AIR TEMPERATURE (TA)
#    using relationships with Mean_Temp, Min_Temp, and Max_Temp
#------------------------------------------------------------------------------
# Fit regression model using rows where TA exists
ta_model <- lm(
  TA ~ Mean_Temp + Min_Temp + Max_Temp,
  data = la2_reduced,
  na.action = na.exclude
)

# Predict missing TA values
la2_reduced_TA_fill <- la2_reduced %>%
  mutate(
    TA_GF = ifelse(
      is.na(TA),
      predict(ta_model, newdata = .),
      TA
    )
  )
...
ggplot() +
  # Filled data only 
  geom_line(
    data = la2_reduced_TA_fill,
    aes(x = DateTime, y = TA_GF, color = "Filled"),
    na.rm = FALSE
  ) +
  # Original data (black)
  geom_line(data = la2_reduced_TA_fill, aes(x = DateTime, y = TA, color = "Original"), na.rm = FALSE) +
  scale_color_manual(values = c("Original" = "black", "Filled" = "red"))



#------------------------------------------------------------------------------
# 2. GAP-FILL WATER TEMPERATURE
#    using the newly gap-filled TA variable
#------------------------------------------------------------------------------

# Fit model between water temperature and TA
wt_model <- lm(
  `Adjusted Water Temperature` ~ TA,
  data = la2_reduced_TA_fill,
  na.action = na.exclude
)

# Fill missing water temperatures
la2_reduced_TA_fill <- la2_reduced_TA_fill %>%
  mutate(
    WT_GF = ifelse(
      is.na(`Adjusted Water Temperature`),
      predict(wt_model, newdata = .),
      `Adjusted Water Temperature`
    )
  )
...
ggplot() +
  ...
  geom_line(
    data = la2_reduced_TA_fill,
    aes(x = DateTime, y = WT_GF, color = "Filled"),
    na.rm = FALSE
  ) +
  # Original data (black)
  geom_point(data = la2_reduced_TA_fill, aes(x = DateTime, y = `Adjusted Water Temperature`, color = "Original"),
             size=0.3, na.rm = FALSE) +
  scale_color_manual(values = c("Original" = "black", "Filled" = "red"))



la2_filtered <- la2_reduced_TA_fill %>%
  filter(DateTime > "2021-01-01") %>%
  filter(!Reduce(`|`,
                 Map(function(start, end) DateTime >= start & DateTime <= end,
                     short_gaps$gap_start, short_gaps$gap_end))) %>%
  select(-c(`Adjusted Water Temperature`,TA))
...
xvars <- train_data %>% select(-c(DateTime,FC,FCH4))


# Impute missing predictors
imputed <- missForest(xvars)

# Completed predictor dataset
xvars_imp <- imputed$ximp

# Combine predictors with response variable

rf_data <- bind_cols(
  DateTime = train_data$DateTime,
  FCH4 = train_data$FCH4,
  xvars_imp
)
...
model <- randomForest(
  ...
  Precip + Mean_Temp + Min_Temp + Max_Temp +
  Sp_Hum + Down_SW_Rad + VPD + WS_Grid + PWD,
  data = rf_data %>% select(-DateTime),  # exclude datetime if needed
  na.action = na.omit,
  importance = TRUE
)
...
df_missing_clean <- la2_filtered %>% filter(is.na(FCH4)) %>% select(-DateTime)
